...
class HOSTPN_Common {
    ...
    if (!wp_style_is($this->plugin_name . '-select2', 'enqueued')) {
			wp_enqueue_style($this->plugin_name . '-select2', HOSTPN_URL . 'assets/css/select2.min.css', [], $this->version, 'all');
    }
    ...
    if (!wp_style_is($this->plugin_name . '-fancybox', 'enqueued')) {
			wp_enqueue_style($this->plugin_name . '-fancybox', HOSTPN_URL . 'assets/css/fancybox.min.css', [], $this->version, 'all');
    }
    ...
    if(!wp_script_is($this->plugin_name . '-select2', 'enqueued')) {
			wp_enqueue_script($this->plugin_name . '-select2', HOSTPN_URL . 'assets/js/select2.min.js', ['jquery'], $this->version, false, ['in_footer' => true, 'strategy' => 'defer']);
    }
    ...
    if(!wp_script_is($this->plugin_name . '-fancybox', 'enqueued')) {
			wp_enqueue_script($this->plugin_name . '-fancybox', HOSTPN_URL . 'assets/js/fancybox.min.js', ['jquery'], $this->version, false, ['in_footer' => true, 'strategy' => 'defer']);
    }
    ...
    if(!wp_script_is($this->plugin_name . '-owl', 'enqueued')) {
		...
		wp_localize_script($this->plugin_name, 'hostpn_ajax', [
			'ajax_url' => admin_url('admin-ajax.php'),
			'ajax_nonce' => wp_create_nonce('hostpn-nonce'),
		]);
		...
		$hostpn_tab = !empty($_GET['hostpn_tab']) ? HOSTPN_Forms::sanitizer(wp_unslash($_GET['hostpn_tab'])) : '';
		...
		wp_localize_script($this->plugin_name, 'hostpn_trumbowyg', [
		...
		wp_localize_script($this->plugin_name, 'hostpn_i18n', [
			...
			'copied' => esc_html(__('Copied to clipboard', 'hostpn')),
		]);
	}

...
class HOSTPN_i18n
{
	...
	public function hostpn_pll_get_post_types($post_types, $is_settings) {
		// Add hostpn_accommodation to Polylang's translatable post types
		// This allows it to appear in Polylang settings and be translated
	...
	/**
	 * Get translated accommodation features array
	 * 
	 * @return array Translated accommodation features
	 */
	public static function hostpn_get_accommodation_features()
	{
		if (!defined('HOSTPN_ACCOMMODATION_FEATURES')) {
			return [];
		}

		$features = HOSTPN_ACCOMMODATION_FEATURES;
		$translated_features = [];

		foreach ($features as $category_key => $category_data) {
			$translated_features[$category_key] = [
				'title' => __($category_data['title'], 'hostpn'),
				'icon' => $category_data['icon'],
				'features' => []
			];

			foreach ($category_data['features'] as $meta_key => $feature_label) {
				$translated_features[$category_key]['features'][$meta_key] = __($feature_label, 'hostpn');
			}
		}

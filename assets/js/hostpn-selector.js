(function($) {
    ...
    class HOSTPN_Selector {
        ...
        createStructure() {
            const wrapper = $('<div class="hostpn-selector"></div>');
            const control = $('<div class="hostpn-selector__control"></div>');
            const valueContainer = $('<div class="hostpn-selector__value-container"></div>');
            const input = $('<input type="text" class="hostpn-selector__input" />');
            const indicator = $('<span class="hostpn-selector__indicator"><i class="material-icons-outlined hostpn-selector__indicator-icon">keyboard_arrow_down</i></span>');
            
            valueContainer.append(input);
            control.append(valueContainer, indicator);
            wrapper.append(control);
            
            this.menu = $('<div class="hostpn-selector__menu" style="display: none;"></div>');
            
            if (this.options.searchable) {
                const searchContainer = $('<div class="hostpn-selector__search"></div>');
            ...
            this.optionsContainer = $('<div class="hostpn-selector__options"></div>');
        ...
        bindEvents() {
            ...
            this.menu.on('click', '.hostpn-selector__option', (e) => {
        ...
        closeMenu() {
            ...
            this.wrapper.find('.hostpn-selector__indicator-icon').text('keyboard_arrow_down');
        }
        ...
        updateOptions() {
            ...
            options.each((_, option) => {
                ...
                const optionElement = $('<div class="hostpn-selector__option"></div>')
                    .text(label)
                    .data('value', value);
        ...
        filterOptions(searchTerm) {
            const options = this.optionsContainer.find('.hostpn-selector__option');
        ...
        addSelectedValue(value, label) {
            ...
            this.valueContainer.find('.hostpn-selector__placeholder').remove();
            if (this.options.multiple) {
                const valueElement = $('<div class="hostpn-selector__multi-value"></div>');
                const labelElement = $('<span class="hostpn-selector__multi-value__label"></span>').text(label);
                const removeButton = $('<span class="hostpn-selector__multi-value__remove"><i class="material-icons-outlined hostpn-icon-close">close</i></span>');
                ...
                removeButton.on('click', (e) => {
                    ...
                    this.optionsContainer.find('.hostpn-selector__option').each(function() {
                ...
                const valueElement = $('<span class="hostpn-selector__single-value"></span>');
                const labelElement = $('<span class="hostpn-selector__single-value__label"></span>').text(label);
                const removeButton = $('<span class="hostpn-selector__single-value__remove"><i class="material-icons-outlined hostpn-icon-close">close</i></span>');
                removeButton.on('click', (e) => {
                    e.stopPropagation();
                    // Clear selection
                    this.selectedValues = [];
                    ...
                    if (this.placeholder) {
                        this.input.hide();
                        this.valueContainer.append('<span class="hostpn-selector__placeholder">' + this.placeholder + '</span>');
                    }
                });
                valueElement.append(labelElement, removeButton);
                this.valueContainer.append(valueElement);
    ...
    $.fn.HOSTPN_Selector = function(options) {
        return this.each(function() {
            if (!$(this).data('-selector')) {
                $(this).data('-selector', new HOSTPN_Selector(this, options));
            }
        });
    };
